...
const totalNum = GAMES_JSON.reduce((total) => {
    return total + 1
}, 0

)
gamesCard.innerHTML = `${totalNum}`
...
const descriptionText = `A total of $${GAMES_JSON.reduce((total, game) => total + game.pledged, 0)
...
topName.textContent = `${topGame.name} - $${topGame.pledged.toLocaleString()}`;
firstGameContainer.appendChild(topName)
// do the same for the runner up item
const secondName = document.createElement("p");
secondName.textContent = `${runnerUp.name} - $${runnerUp.pledged.toLocaleString()}`;
